...
Dette eksempel er et supplement til indholdet på forelæsningsslides fra d. 30/9-2020.
...
* Hvis `Girth` bliver dobbelt så stor, så øges `log(Girth)` med `log(2)`=0.693, og så øges medianen for `Volume` med en faktor `exp(0.693*1.983)`=3.95. I grove træk vil en fordobling af diameteren (`Girth`) altså modsvares af en fordobling af medianen for `Volume` (hvis ellers højden er uændret).
...
Som beskrevet på slides 29-31 fra forelæsning d. 21/9-2020 (og i lærebogens Ch. 6.4), så kan vi teste hypotesen $H_0: \beta_1 = 1, \beta_2 = 2$ ved at fortage en sammenligne (med `anova()`) af
...
$$1 \cdot \texttt{log(Height)}_i + 2 \cdot \texttt{log(Girth)}_i$$
 ned over modellen, så R kun estimerer intercept-parameteren ($\alpha$). Den estimerede model (nulmodel) sammenlignes med den fuld (dvs. urestringede multiple lineære regressionsmodel) ved et F-test.
...
Under hypotesen $H_0$ så bør den modificerede respons `log(Volume) - log(Height) - 2*log(Girth)` slet ikke afhænge af hverken `log(Height)` eller `log(Girth)`. Derfor fittes den restringede model (`nulModel`) ved blot at estimere et intercept (svarende til formelsyntaksen `~ 1`). De to modeller testes imod hinanden ved et kald til `anova()`.
