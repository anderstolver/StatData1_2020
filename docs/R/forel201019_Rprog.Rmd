...
# StatData1-studerendes svar på spørgsmål 11 i quiz til kursusuge 6

Der ver 29 studerende, som har deltaget i quizzen og havde mulighed for at besvare spørgsmålet. Der var 6 mulige svarkategorier og 8 af de 29 studerende svarede korrekt.

Vi lader $Y$ betegne antallet af studerene, som svarer korrekt og benytter den statistiske model, hvor $Y \sim bin(29, p)$ hvor parameteren p der beskriver successandsynligheden (i hver *trial*) opfattes som ukendt.

## Estimat og simpelt KI 

Vi laver beregningerne manuelt (baseret på s. 316-317 i+ Ex. 11.7 i lærebogen) og får $\hat p=0.276$ og 95% konfidensinterval (0.113, 0.439).
...
Vi beregner først det forbedrede KI manuelt (lærebogens kapitel 11.3.1) og får (0.146, 0.460).

```{r}
p1tilde <- (8+2)/(29+4)
p1tilde - 1.96 * sqrt(p1tilde * (1-p1tilde) / 33)
p1tilde + 1.96 * sqrt(p1tilde * (1-p1tilde) / 33)
```

Derefter bruger vi `prop.test` og får (0.147, 0.457).

Bemærk: 

* De to metoder giver ikke helt det samme, fordi den approksimation der benyttes af `prop.test` er lidt anderledes. Begge dele er fornuftigt nok, og I kan gøre det I foretrækker. 

* Bemærk at `prop.test` også giver en p-værdi. Hvis man ikke angiver en hypoteseværdi, så testes hypotesen $H_0: p=0.5$ som er helt irrelevant i dette tilfælde. Mere om dette senere i R programmet

```{r}
prop.test(8, 29, correct=FALSE)
```

## Test af hypotese 

* Der var seks svarmuligheder, så hvis studerende blot gætter svarer dette til at p=1/6. Vi tester derfor hypotesen $H_0: p=1/6$

* Vi benytter observationen 8 selv som teststørrelse og summerer alle de punktsandsynligheder der er $\leq P(Y=8)$ 
...
```{r}
prop.test(8, 29, correct=FALSE, p=1/6)
```
...
Eksemplet er diskuteres på slides til forelæsningen d. 19/10-2020 på Statistisk Dataanalyse 1
