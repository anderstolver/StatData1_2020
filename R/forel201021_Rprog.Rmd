...
Eksemplet blev første gang diskuteret ved forelæsningen d. 19/10-2020 på Statistisk Dataanalyse 1
...
For en 2x2 tabel laver `chisq.test` som default en kontinuitetskorrektion. Hvis vi ikke vil have det, skal vi bruge optionen `correct=FALSE`. Ved forelæsningerne snakker vi kun om versionen uden kontinuitetskorrektion (-se dog slide 44-45 fra 23/10-2020).
...
# Transporttid til Campus og foretrukken undervisningsform

## Beskrivelse af data

Ved første forelæsning i Statistisk Dataanalyse 1 d.31/8-2020 blev der indsamlet diverse informationer omkring nogle af de studerende, som var tilstede ved forelæsningen. 
...
* Hvilket udsagn passer bedst på dig?
    - Jeg ønsker så meget **fysisk** undervisning på Frederiksberg Campus som muligt
    - Jeg ønsker en **blanding** af online undervisning og fysisk undervisning på Frederiksberg Campus
    - Jeg ønsker at følge alle undervisningsaktiviteter **online**
    
Data til dette eksempel kan findes under menupunktet **Data** på kursushjemmesiden. 

Nedenfor inddeler svarene fra 175 studerende efter de to inddelingskriterier

* `transportid`: over eller under 20 minutter
* `uv`: foretrukken undervisningsform fysisk, online eller en blanding

Disse data kan opsummeres i følgende antalstabel.

```{r}
sd1data <- read.table(file = "../data/sd12020.txt", header = T)
sd1tabel <- table(cut(sd1data$transporttid, breaks = c(0, 20, 180)), sd1data$uv)
sd1tabel
```
...
Vi konkluderer at  hypotesen om at de to inddelingskriterier er uafhængige forkastes ($\chi^2 = 24.304$, p stort set lig med 0).
...
**Konklusion:** Det virker især til, at studerende som bor langt fra Campus er mere tilbøjelige til at foretrække online (frem for fysisk) undervisning, end studerende som bor tæt på Campus. Min personlige fortolkning er, at studerende i højere grad har svaret på, hvilken undervisningsform der er mest praktisk / bekvem for dem, end på hvilken undervisningsform der giver dem det bedste udbytte.

**Simulering af p-værdi ved tynde tabeller:** Man kan komme ud for, at `chisq.test` giver en advarsel (`Warning`), hvis man forsøger at udføre et test på en såkaldt *tynd tabel*, hvor der er få observationer i nogle af cellerne. Nedenfor gives et eksempel, hvor dette er tilfældet. For at opnå dette benyttes en anden opdeling/gruppering af transporttiden i grupperne 0-60 minutter eller over 60 minutter.

```{r}
new_sd1tabel <- table(cut(sd1data$transporttid, breaks = c(0, 60, 180)), sd1data$uv)
new_sd1tabel
chisq.test(new_sd1tabel)
```

Når vi bruger `chisq.test`, får vi en p-værdi på 0.0340, men vi får også en advarsel (`Warning`) om at $\chi^2$- approksimationen er tvivlsom. Det er fordi nogle af de forventede værdier er mindre end 5 (-se tabel nedenfor). Vi er derfor ikke ganske sikre på validiteten af p-værdien.
...
Så får vi en p-værdi på omkring 0.0306. Bemærk at vi ikke får præcis det samme hver gang, medmindre vi som i R koden vælger et fast `seed` (her vha. `set.seed(2019)`)
...
p-værdierne fra de to forskellige metoder er ret ens (0.0340 vs cirka 0.0306), så vi får samme konklusion.
